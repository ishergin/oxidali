...
Изменения попадают в ветку через pull request; как его подготовить, в том числе для
изменения web UI, — в [`CONTRIBUTING.md`](CONTRIBUTING.md). Новое поведение начинается с
падающего BDD-сценария. Перед тем как изменение попадёт в ветку, `just ci` должен быть
зелёным. Правила — в [`CLAUDE.md`](CLAUDE.md); рецепты
