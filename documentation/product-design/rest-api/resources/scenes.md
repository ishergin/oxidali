...
- Чужой мастер, программирующий сцену, сбрасывает `applied` затронутых строк: после
  его `SET SCENE` уровень не прочитан, после `REMOVE FROM SCENE` прибор не в сцене, а
  при неизвестном членстве в группе — тоже не прочитан. Строка, чей `desired` с этим
  расходится, становится `dirty` до следующего apply или чтения уровней.
