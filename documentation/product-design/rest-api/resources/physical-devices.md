...
- `memory_bus_unit` — банк 0 выше `0x1A`: байт конфигурации с классификацией и маска
  реализованных Part 15x, где известно, сколько байт маски ответило (старшие биты
  неизвестны, а не нули). Нет секции — прибор молчит на эти локации, что конформно.
...
- Каждая запись проверяется read-back'ом, и в реестр попадает **принятое** прибором, а
  ...
  проводе ничего не сломалось. Read-back без ответа 16-битный путь записи сейчас
  принимает за подтверждение (открытый ISSUE-118 в
  [`known-issues.md`](../../known-issues.md)).
- Гейта по типу прибора у Tc-лимитов и кривой нет
  ([09 §Device types](../../../architecture/09-dali-protocol-rules.md#device-types-and-memory-banks)):
  Tc-лимиты прибора без DT8 остаются без подтверждения, а кривая прибора без DT6 из-за
  ISSUE-118 его получает. Контролы прячет UI.
