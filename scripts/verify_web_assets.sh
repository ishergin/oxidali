#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

APP_DIR="$ROOT/web/app"
FW_ASSETS_DIR="$ROOT/crates/dali2rust-firmware/assets/web"
MANIFEST_RS="$ROOT/crates/dali2rust-firmware/src/web_assets.rs"
STAMP_FILE="$FW_ASSETS_DIR/.sources.sha256"

die() {
  echo "verify_web_assets: $*" >&2
  exit 1
}

source_fingerprint() {
  (
    cd "$APP_DIR"
    find src public -type f -print0 2>/dev/null
    printf '%s\0' index.html package.json package-lock.json vite.config.ts
    printf '%s\0' tsconfig.json tsconfig.app.json tsconfig.node.json
  ) | LC_ALL=C sort -z | while IFS= read -r -d '' rel; do
    [[ -f "$APP_DIR/$rel" ]] || continue
    printf '%s ' "$rel"
    shasum -a 256 "$APP_DIR/$rel" | cut -d' ' -f1
  done | shasum -a 256 | cut -d' ' -f1
}

[[ -d "$APP_DIR" ]] || die "missing web app dir: $APP_DIR"
[[ -f "$MANIFEST_RS" ]] || die "missing manifest: $MANIFEST_RS"

missing=()
while IFS= read -r rel; do
  [[ -f "$FW_ASSETS_DIR/$rel" ]] || missing+=("$rel")
done < <(grep -o 'asset_bytes!("[^"]*")' "$MANIFEST_RS" | sed 's/asset_bytes!("//; s/")//')

if ((${#missing[@]} > 0)); then
  echo "verify_web_assets: manifest embeds files absent from the mirror:" >&2
  printf '  %s\n' "${missing[@]}" >&2
  die "run scripts/build_web_ui.sh and commit $FW_ASSETS_DIR"
fi

if [[ ! -f "$STAMP_FILE" ]]; then
  die "missing source stamp $STAMP_FILE — run scripts/build_web_ui.sh and commit the mirror"
fi

expected="$(source_fingerprint)"
recorded="$(tr -d '[:space:]' <"$STAMP_FILE")"

if [[ "$expected" != "$recorded" ]]; then
  echo "verify_web_assets: web/app sources changed but the firmware mirror did not." >&2
  echo "  recorded: $recorded" >&2
  echo "  current:  $expected" >&2
  die "run scripts/build_web_ui.sh and commit $FW_ASSETS_DIR"
fi

INDEX_GZ="$FW_ASSETS_DIR/index.html.gz"
[[ -f "$INDEX_GZ" ]] || die "missing mirrored index: $INDEX_GZ"

asset_refs="$(gunzip -c "$INDEX_GZ" | grep -oE '/assets/[^"]+' || true)"
if [[ -z "$asset_refs" ]]; then
  die "mirrored index references no /assets/ file — build_web_ui.sh output changed shape"
fi

while IFS= read -r ref; do
  [[ "$ref" == *"?v="* ]] && continue
  echo "verify_web_assets: unstamped asset reference in the mirrored index:" >&2
  echo "  $ref" >&2
  die "assets are served immutable and MUST carry ?v=<hash> — check the sed in scripts/build_web_ui.sh"
done <<<"$asset_refs"

if [[ -d "$APP_DIR/node_modules" ]]; then
  (cd "$APP_DIR" && npx --no-install tsc -b --noEmit 2>/dev/null || npx --no-install tsc -b) \
    || die "tsc -b failed in web/app"
  (cd "$APP_DIR" && npm test) || die "frontend tests failed in web/app"
  echo "verify_web_assets: OK (manifest, mirror fresh, ?v= stamped, tsc and tests clean)."
else
  if [[ "${DALI2RUST_SKIP_TSC:-0}" != "1" ]]; then
    die "web/app/node_modules is missing, so nothing type-checked the web UI. \
Run 'npm ci' in web/app, or set DALI2RUST_SKIP_TSC=1 to say out loud that this \
run does not check it."
  fi
  echo "verify_web_assets: OK (manifest and mirror clean; tsc/tests SKIPPED by DALI2RUST_SKIP_TSC=1)."
fi
